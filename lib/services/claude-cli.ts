...
export interface SpawnClaudeCliOptions {
  ...
  externalDirs?: string[];
  /**
   * Set when `cwd` is a WSL-side path (from `getWslDistro`) — a Windows-side
   * spawn can't `cd` into it (cmd.exe rejects UNC paths as a cwd outright).
   * Routes through `wsl -d <distro> -- bash -lc` instead, which sources
   * .bashrc/.profile so PATH additions (nvm, ~/.local/bin) still resolve `claude`.
   */
...
export function spawnClaudeCli(opts: SpawnClaudeCliOptions): ChildProcessWithoutNullStreams {
  if (opts.wslDistro && opts.cwd) {
    // The settings file (hook mode) and any --add-dir targets are written/
    // resolved by this (Windows) process at Windows paths — the WSL-side
    // claude process needs them through the /mnt/<drive> mount instead.
    // No-op on paths that are already POSIX (e.g. a WSL-sourced skill's own
    // external dirs), since the regex only matches a drive-letter prefix.
    const wslOpts: SpawnClaudeCliOptions = {
      ...opts,
      permission: opts.permission.mode === 'hook'
        ? { ...opts.permission, settingsPath: toWslPath(opts.permission.settingsPath) }
        : opts.permission,
      externalDirs: opts.externalDirs?.map(toWslPath),
    };
    const args = buildClaudeCliArgs(wslOpts);
    return spawn('wsl', ['-d', opts.wslDistro, '--cd', opts.cwd, '--', 'bash', '-lc', ['claude', ...args].join(' ')], {
      shell: false,
      env: { ...process.env },
