import type { SkillRef } from '@/lib/services/improvement-prompt';

// Content for these two skills is vendored here rather than read from
// ~/.claude/skills at request time. That path is only valid on the host
// machine that originally ran `claude` — inside the Docker container
// os.homedir() resolves to the container user's home, which never has a
// .claude/skills directory, and a live filesystem read would silently point
// at nothing. Baking the content into lib/ (which the Dockerfile already
// copies wholesale into the runner stage) makes these two skills always
// available regardless of where the improvement prompt gets generated.
//
// These are pinned snapshots, not live pointers — if grill-me or
// writing-great-skills change upstream, re-copy their content here.
...
const WRITING_GREAT_SKILLS_CONTENT = `A skill exists to wrangle determinism out of a stochastic system. **Predictability** — the agent taking the same _process_ every run, not producing the same output — is the root virtue; every lever below serves it.
...
const FIXED_SKILLS: Record<string, { name: string; content: string }> = {
  'grill-me': { name: 'grill-me', content: GRILL_ME_CONTENT },
  'writing-great-skills': { name: 'writing-great-skills', content: WRITING_GREAT_SKILLS_CONTENT },
};

export const FIXED_SKILL_IDS = Object.keys(FIXED_SKILLS);

// Resolves selected skill ids into refs the prompt generator can render:
// fixed skills carry their vendored content inline; anything else is looked
// up against the skills this session was actually observed to invoke, and
// stays a path reference (so it still needs an --add-dir Read grant).
export function resolveSelectedSkills(
  skillIds: string[],
  invokedSkills: { name: string; dir: string }[],
): SkillRef[] {
  const invokedByName = new Map(invokedSkills.map(s => [s.name, s.dir]));
  const resolved: SkillRef[] = [];
  for (const id of skillIds) {
    const fixed = FIXED_SKILLS[id];
    if (fixed) {
      resolved.push({ kind: 'inline', name: fixed.name, content: fixed.content });
      continue;
    }
    const dir = invokedByName.get(id);
    if (dir) resolved.push({ kind: 'path', name: id, dir });
  }
  return resolved;
}
