    ...
    <div className="flex flex-col h-full bg-[#010409]">
      ...
      <div className="flex items-center gap-3 px-4 py-2.5 border-b border-[#21262d] bg-[#0d1117] shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-semibold text-[#e6edf3] truncate">{fileName}</div>
          <div className="text-[10px] text-[#484f58] font-mono truncate mt-0.5">{data.filePath}</div>
        </div>

        {/* Metadata chips */}
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-[11px] text-[#8b949e] bg-[#21262d] px-2 py-0.5 rounded font-mono">
            {data.lang}
          </span>
        ...
        {canRender && (
          <div className="flex rounded border border-[#30363d] overflow-hidden shrink-0">
            ...
            <button
              ...
              className={cn(
                'flex items-center gap-1.5 px-2.5 py-1 text-[11px] border-l border-[#30363d] transition-colors',
                viewMode === 'source'
                  ? 'bg-[#21262d] text-[#e6edf3]'
                  : 'text-[#8b949e] hover:text-[#e6edf3]'
              )}
        ...
        <button
          onClick={copy}
          className="flex items-center gap-1.5 px-2.5 py-1 rounded border border-[#30363d] text-[11px] text-[#8b949e] hover:text-[#e6edf3] hover:border-[#484f58] transition-colors shrink-0"
        >
      ...
      {/* Document body */}
      <ScrollArea className="flex-1 bg-[#010409]">
        {isMarkdown && viewMode === 'rendered' ? (
          /* Markdown document view — centered paper */
          <div className="py-10 px-6">
            <div
              className="mx-auto rounded-lg border border-[#21262d] bg-[#0d1117] shadow-2xl overflow-hidden"
...
function DocumentContent({ content }: { content: string }) {
  ...
  return (
    <article className="px-10 py-10 text-[15px] leading-7 text-[#e6edf3]">
...
function renderInline(text: string): React.ReactNode {
  ...
  while ((m = regex.exec(text)) !== null) {
    ...
    else if (t.startsWith('`')) parts.push(<code key={m.index} className="font-mono text-[13px] px-1.5 py-0.5 rounded bg-[#161b22] text-[#f0883e] border border-[#21262d]">{t.slice(1, -1)}</code>);
    else if (t.startsWith('[')) {
      const label = t.match(/\[([^\]]+)\]/)?.[1] || '';
      const href = t.match(/\(([^)]+)\)/)?.[1] || '';
      parts.push(<a key={m.index} href={href} className="text-[#58a6ff] hover:underline">{label}</a>);
    }
...
function renderSection(s: MarkdownSection, key: number): React.ReactNode {
  switch (s.type) {
    case 'h1':
      return (
        <h1 key={key} className="text-[26px] font-bold text-[#e6edf3] mb-4 mt-0 pb-3 border-b border-[#21262d]">
          {renderInline(s.content)}
        </h1>
      );
    case 'h2':
      return (
        <h2 key={key} className="text-[20px] font-semibold text-[#e6edf3] mt-8 mb-3 pb-2 border-b border-[#21262d]">
          {renderInline(s.content)}
        </h2>
      );
    case 'h3':
      return <h3 key={key} className="text-[16px] font-semibold text-[#e6edf3] mt-6 mb-2">{renderInline(s.content)}</h3>;
    case 'h4':
      return <h4 key={key} className="text-[14px] font-semibold text-[#c9d1d9] mt-4 mb-1">{renderInline(s.content)}</h4>;
    case 'p':
      return <p key={key} className="text-[#c9d1d9] leading-7 mb-4">{renderInline(s.content)}</p>;
    case 'ul':
      return (
        <ul key={key} className="mb-4 space-y-1.5 pl-1">
          {(s.items || []).map((item, i) => (
            <li key={i} className="flex items-start gap-2.5 text-[#c9d1d9]">
              <span className="mt-2.5 w-1.5 h-1.5 rounded-full bg-[#484f58] shrink-0" />
    ...
    case 'ol':
      return (
        <ol key={key} className="mb-4 space-y-1.5 pl-1">
          {(s.items || []).map((item, i) => (
            <li key={i} className="flex items-start gap-2.5 text-[#c9d1d9]">
              <span className="text-[#484f58] font-mono text-sm shrink-0 mt-0.5">{i + 1}.</span>
              <span>{renderInline(item)}</span>
            </li>
          ))}
        </ol>
    ...
    case 'code-block':
      return (
        <div key={key} className="mb-4 rounded-md overflow-hidden border border-[#30363d]">
          {s.lang && (
            <div className="flex items-center justify-between px-4 py-1.5 bg-[#161b22] border-b border-[#30363d]">
              <span className="text-[11px] font-mono text-[#8b949e]">{s.lang}</span>
            </div>
          )}
          <pre className="p-4 overflow-x-auto bg-[#161b22] text-[13px] font-mono text-[#c9d1d9] leading-5 whitespace-pre">
            {s.content}
          </pre>
        </div>
      );
    case 'blockquote':
      return (
        <blockquote key={key} className="mb-4 pl-4 border-l-4 border-[#58a6ff]/40 text-[#8b949e] italic">
          {renderInline(s.content)}
        </blockquote>
      );
    case 'hr':
      return <hr key={key} className="my-6 border-[#21262d]" />;
    case 'table':
      return (
        <div key={key} className="mb-4 overflow-x-auto rounded-md border border-[#30363d]">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#161b22]">
                {(s.headers || []).map((h, i) => (
                  <th key={i} className="px-4 py-2 text-left text-[#e6edf3] font-semibold border-b border-[#30363d]">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {(s.rows || []).map((row, ri) => (
                <tr key={ri} className="border-b border-[#21262d] hover:bg-[#161b22]/50">
                  {row.map((cell, ci) => (
                    <td key={ci} className="px-4 py-2 text-[#c9d1d9]">
                      {renderInline(cell)}
                    </td>
                  ))}
                </tr>
...
function CodeContent({ lines, lang }: { lines: string[]; lang: string }) {
  return (
    <div className="flex text-[13px] font-mono leading-5 min-h-full">
      {/* Line numbers gutter */}
      <div
        className="select-none text-right border-r border-[#21262d] sticky left-0 z-10 bg-[#010409]"
        style={{ minWidth: '52px', padding: '16px 12px 16px 0' }}
      >
        {lines.map((_, i) => (
          <div key={i} className="text-[#484f58] leading-5 px-2">{i + 1}</div>
        ))}
      </div>
      {/* Code content */}
      <pre className="flex-1 p-4 text-[#c9d1d9] whitespace-pre-wrap leading-5 break-words overflow-x-auto">
        {lines.join('\n')}
      </pre>
    </div>
  );
